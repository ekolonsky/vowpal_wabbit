...
from sklearn.metrics import roc_curve, auc, log_loss, precision_recall_curve, accuracy_score
import numpy as np
...
def read_arguments():
    ...
    parser.add_argument('--outer_loss_function', default='logistic',
                        choices=['logistic', 'roc-auc', 'accuracy'])  # TODO: implement squared, hinge, quantile, PR-auc
...
class HyperOptimizer(object):
    ...
    def get_y_true_holdout(self):
        ...
        self.y_true_holdout = []
        for line in yh:
            self.y_true_holdout.append(int(line.split()[0]))
    ...
    def validation_metric_vw(self):
        ...
        y_pred_holdout = []
        for line in v:
            y_pred_holdout.append(int(line.split()[0].strip()))
        ...
        elif self.outer_loss_function == 'accuracy':  # TODO: write it
            #print 'y_true_holdout', self.y_true_holdout[0:4]
            #print 'y_pred_holdout', y_pred_holdout[0:4]
            loss = 1 - accuracy_score(self.y_true_holdout, y_pred_holdout)
            print 'Holdout accuracy %f'%(1-loss)
